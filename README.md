...
This repository is the **Debian** build — a `.deb`, X11 input and capture. Other platforms live in their own repositories, each stripped to
one target.

## Install

```bash
git clone https://github.com/Ace-Haxxed/Aria-Debian
cd Aria-Debian
...
The first script installs system dependencies with `apt`. The second builds a
release binary and puts `aria` in `/usr/local/bin`.
...
**Needs:** Node 18+, Rust 1.77+, and the system dependencies the install script
handles.
...
src-tauri/
  ...
  src/platform/  detect.rs, x11.rs, portal.rs
```
...
**Input and capture use the X11 tools** — `xdotool` for mouse and keyboard,
`scrot` for screenshots, `wmctrl` for window management. On a Wayland session
these keep working through XWayland, which is what Debian's own desktop
packages rely on.
...
- **Wayland sessions run through XWayland.** This build ships the X11 backend
  only. Debian 12's GNOME defaults to Wayland, where XWayland covers input and
  capture for X11 clients but window management is limited.
- **Screen capture on GNOME** may go through xdg-desktop-portal and ask for
